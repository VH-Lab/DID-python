...
import struct
import random
from .globals import did_documentpath, did_schemapath
...
class DIDDocument:
	...
	def make_blankdocument(cls, document_type):
		...
		def __make_id__():
			...
			serial_date_number = hex(struct.unpack('!q', struct.pack('!d', integral_part + fractional_part))[0])
			random_number = random.random() + random.randint(-32727, 32727)
			hex_random_number = hex(struct.unpack('!q', struct.pack('!d', random_number))[0])
			return "{}_{}".format(serial_date_number, hex_random_number)

		data = {'base':
			{
				'id': '',
				'session_id': __make_id__(),
				'name': '',
				'datestamp': '',
				'database_version': 1
			},
	...
	def from_json(cls, document_type, starting_path=did_documentpath()):
		...
		def __readjsonfromblankfile__(jsonstr):
			...
			for superclass in properties.get('document_class', default={'superclass': []}).get('superclasses',
																							   default=[]):
				doc = cls.from_json(document_type, starting_path)
				properties[superclass] = doc.serialize()[superclass]
			return properties

		if not document_type.endswith('.json'):
			document_type += '.json'
		try:
			f = open(os.path.join(starting_path, document_type))
			try:
				properties = __readjsonfromblankfile__(f.read())
				return cls(data={'document_properties': properties})
			finally:
				f.close()
		except FileNotFoundError:
			dirpath, fname = __search__(starting_path, document_type)
			return cls.from_json(document_type=fname, starting_path=dirpath)
	...
	def superclasses(self):
		return self.data.get('class').get('super_classes')
