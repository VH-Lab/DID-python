...
INSTALLATION = os.path.split(os.path.abspath(did.__name__))[0]
...
def get_database_connection_string(db_name):
    if db_name == 'mongodb':
...
def set_database_connection_string(db_name):
    if db_name == 'mongodb':
        return get_variable("MONGO_CONNECTION_STRING")
