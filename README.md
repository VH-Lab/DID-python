...
## DID Schema Dependency
DID Python relies on the flatbuffers generated by DID Schema. To link this dependency, follow these steps:
...
3. Run the command `ln -s <path-to-ndi-schema-directory>/build/ndi/scripts ./ndi` from within the ndi-python directory. eg. if ndi-schema directory is called 'DID-schema' and it's in the same directory as ndi-python, then run `ln -s ../DID_schema/build/ndi/scripts ./ndi`.

## Package manager
Ndi-python uses pipenv is used for package management, it can be installed with pip.f
...
DID_Object subclasses are PascalCase, their collection/table names are snake_case + 's', and their reference keys are snake_case + '_id'.
