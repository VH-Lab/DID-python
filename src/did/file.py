...
class BinaryTable:
    ...
    def write_header(self, header_data):
        if len(header_data) > self.header_size:
            raise ValueError(
                "Header data to write is larger than the header size of the file."
            )
...
class FileCache:
    ...
    def __init__(
        self, directory_name, file_name_characters=32, max_size=100e9, reduce_size=80e9
    ):
    ...
    def _load_properties(self):
        ...
        self.file_name_characters = info.get(
            "fileNameCharacters", self.file_name_characters
        )
