...
.. glossary::
  ...
  flatbuffer class
    These are generated by the flatbuffer schema compiler `flatc <https://google.github.io/flatbuffers/flatbuffers_guide_using_schema_compiler.html>`_ when run with the :code:`--python` flag. The :term:`DID schema` that build our flatbuffer classes can be found at `ndi_schema <>`_.

  .. currentmodule:: ndi.ndi_object
  DID class
    A class derived from :class:`DID_Object`, whose purpose is to act as a wrapper for a :term:`flatbuffer class`. It serves as simplified means of quickly converting between an :term:`DID object` and and its :term:`flatbuffer` bytearrays. DID classes are named in PascalCase.

  DID object
    An instance of an :term:`DID class`.

  DID ID
    Every :term:`DID object` should have a unique identifier. This string is automatically generated when a new object is instantiated if an id is not given. IDs are preserved between DID objects and the associated :term:`document`\ s in database.
  ...
  collection
    ...
    .. note::
       Collections are standardly named after their associated :term:`DID class`, only in snake_case and with an appended 's' (eg. 'DaqSystem' -> 'daq_systems' or 'DIDClass' -> 'ndi_classs'). We use :func:`ndi.utils.class_to_collection_name` for consistent conversion.
  ...
  payload
    A dict whose keys are a subset of the fields in a given :term:`collection`, and whose values must comply with the types defined for those fields in the corresponding table defined by the `ndi-schema <>`_. For example, the following :term:`DID schema`:
    ::
  ...
  SQLA query
    The SQL Alchemy equivalent of an :term:`DID query` (`docs <https://www.tutorialspoint.com/sqlalchemy/sqlalchemy_orm_filter_operators.htm>`_). Not to be confused with the terms :term:`SQL query` or :term:`SQLA session query`.
