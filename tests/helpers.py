            ...
            if field in doc.document_properties:
                if field in doc_here.document_properties:
                    ...
                    if not eq_len(field1, field2):
                        return (
                            False,
                            f"Field {field} of document {id_here} did not match.",
                        )
                else:
                    return (
                        False,
                        f"Field {field} not found in document {id_here} from the database.",
                    )
...
def make_tree(n_initial, children_rate, children_rate_decay, max_depth):
    ...
    for i in range(n_initial):
        ...
        if num_children_here > 0:
            sub_g = make_tree(
                num_children_here,
                children_rate * children_rate_decay,
                children_rate_decay,
                max_depth - 1,
            )
...
def verify_branch_node_structure(db, g, node_names):
    ...
    for node, node_name in node_names.items():
        ...
        expected_parents = (
            [node_names[p] for p in g.predecessors(node)] if node in g else []
        )
        expected_children = (
            [node_names[c] for c in g.successors(node)] if node in g else []
        )
        ...
        if set(expected_parents) != set(actual_parent):
            return (
                False,
                f"Error in parent of {node_name}. Expected {expected_parents}, got {actual_parent}",
            )
        ...
        if set(expected_children) != set(actual_children):
            return (
                False,
                f"Error in sub_branch of {node_name}. Expected {expected_children}, got {actual_children}",
            )
