...
# These are storage-layer tests: they exercise _do_add_doc's transaction and
# OnDuplicate handling, not schema validation, so every add passes
# validate=False. The stub documents below carry a synthetic class ('thing')
# with no schema definition and short non-UID ids, both of which
# database.validate_docs rejects before _do_add_doc is ever reached.
